...
const Dashboard = () => {
  ...
  const handleCalculateScores = async () => {
      if(!league) return;
      if(!window.confirm('1. هل تريد حساب نقاط اللاعبين وتطبق نظام العقوبات والمكافآت؟')) return;
      try { 
  ...
  return (
    <div className="dashboard-container" style={{ padding: '40px', fontFamily: 'Arial, sans-serif', direction: 'rtl' }}>
      ...
      <header className="header-res" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px', borderBottom: '1px solid #ddd', paddingBottom: '20px' }}>
        ...
        <button onClick={logout} className="logout-btn-res" style={{ backgroundColor: '#ff4d4d', color: 'white', border: 'none', padding: '10px 20px', cursor: 'pointer', height: '40px', borderRadius: '8px' }}>تسجيل خروج</button>
      </header>
        ...
        <div style={{ backgroundColor: '#f3e5f5', padding: '20px', borderRadius: '10px', marginBottom: '40px', border: '1px solid #ce93d8' }}>
          ...
          <div className="admin-grid-res" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
             ...
             <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <h3 style={{marginTop:0, color:'#38003c'}}>1. إعدادات البطولة</h3>
              {league ? (
                  <div>
                      <p><strong>البطولة:</strong> {league.name}</p>
            ...
            <div style={{ backgroundColor: 'white', padding: '15px', borderRadius: '8px', opacity: league ? 1 : 0.5 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <button onClick={handleGenerateFixtures} disabled={!league} style={{ padding: '10px', width: '100%', backgroundColor: '#00ff85', color: '#38003c', border: 'none', fontWeight: 'bold', borderRadius:'8px', cursor: league ? 'pointer' : 'not-allowed' }}>توليد جدول المباريات (CSV)</button>
            ...
            <div className="admin-tables-box-res" style={{ gridColumn: '1 / -1', display: 'flex', gap: '20px' }}>
                ...
                <div style={{ flex: 1, backgroundColor: '#e3f2fd', padding: '15px', borderRadius: '8px', border: '1px solid #90caf9' }}>
                    <h3 style={{ margin: '0 0 10px 0', color: '#1565c0' }}>🔄 تغيير لاعبين</h3>
                                ...
                                {pendingSubs.map(team => (
                                    <tr key={team._id} style={{ borderBottom: '1px solid #eee' }}>
                                        <td style={{ padding: '10px' }}>
                                            ...
                                            <div style={{fontSize:'11px', color:'#d32f2f'}}>طرد: {team.substitutionRequest.memberName}</div>
                                        </td>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px', marginTop: '20px' }}>
                <div style={{ border: '1px solid #ccc', padding: '20px', borderRadius: '8px', textAlign:'center' }}>
                    ...
                    <div className="input-row-res" style={{display:'flex', justifyContent:'center', gap:'10px'}}>
                        <input type="text" placeholder="كود البطولة" value={leagueCode} onChange={(e) => setLeagueCode(e.target.value)} style={{ padding: '10px', width: '200px' }} />
                        <button onClick={handleJoinLeague} style={{ padding: '10px 20px', backgroundColor: '#38003c', color: 'white', border: 'none', borderRadius:'8px' }}>انضمام</button>
                    </div>
                </div>
            </div>
        )}
                    ...
                    <div style={{ border: '1px solid #ccc', padding: '20px', borderRadius: '8px', backgroundColor: '#f9f9f9', textAlign:'center' }}>
                        ...
                        {selectionMode === 'create' && (
                            <div style={{ marginTop: '20px' }}>
                                ...
                                <div style={{ marginTop: '20px' }}>
                                    <button onClick={() => setSelectionMode(null)} style={{ padding: '10px 20px', background: '#ccc', border: 'none', borderRadius: '5px' }}>إلغاء</button>
                                    <button onClick={handleCreateTeam} disabled={!selectedTeam} style={{ padding: '10px 40px', backgroundColor: '#38003c', color: 'white', border: 'none', borderRadius: '5px', marginRight:'10px', fontWeight:'bold' }}>تأكيد</button>
                                </div>
                            </div>
                        )}
                        {selectionMode === 'join' && (
                            <div style={{ marginTop: '20px' }}>
                                    ...
                                    {leagueTeams.map((team) => {
                                        ...
                                        return (
                                            <div key={team._id} onClick={() => !isFull && setTargetTeamId(team._id)} style={{ border: isSelected ? '3px solid #00ff85' : '1px solid #eee', borderRadius: '12px', padding: '10px', cursor: isFull ? 'not-allowed' : 'pointer', backgroundColor: isSelected ? '#f1f8e9' : 'white', textAlign: 'center', position:'relative' }}>
                                                ...
                                                <div style={{ fontSize: '11px', color: isFull ? 'red' : 'green' }}>{isFull ? 'ممتلئ' : `متاح (${team.members?.length}/4)`}</div>
                                                {isSelected && <FaCheckCircle style={{ position:'absolute', top:'5px', right:'5px', color:'#00c853' }} />}
                                            </div>
                                        );
                                    })}
                                ...
                                <div style={{ marginTop: '20px' }}>
                                    <button onClick={() => setSelectionMode(null)} style={{ padding: '10px 20px', background: '#ccc', border: 'none', borderRadius: '5px' }}>إلغاء</button>
                                    <button onClick={handleJoinTeamRequest} disabled={!targetTeamId} style={{ padding: '10px 40px', backgroundColor: '#38003c', color: 'white', border: 'none', borderRadius: '5px', marginRight:'10px', fontWeight:'bold' }}>انضمام</button>
                                </div>
                            </div>
                        )}
                    </div>
      ...
      {showSubModal && myTeamData && (
          <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.7)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:2000}}>
              <div style={{background:'white', padding:'30px', borderRadius:'15px', width:'90%', maxWidth:'450px', textAlign:'center'}}>
                  <h3 style={{marginTop:0, color:'#38003c'}}>إدارة الفريق</h3>
                  ...
                  <button onClick={() => setShowSubModal(false)} style={{marginTop:'20px', padding:'10px 30px', background:'#333', color:'white', border:'none', borderRadius:'8px', cursor:'pointer'}}>إغلاق</button>
              </div>
          </div>
      )}
