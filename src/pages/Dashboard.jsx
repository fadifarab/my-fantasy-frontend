...
import { 
    ...
    FaLock, FaTimes, FaCheck, FaCrown, FaExclamationTriangle, FaSkullCrossbones, FaInfoCircle, FaFileExcel, FaMagic, FaListOl
} from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const [manualGw, setManualGw] = useState(1);
  ...
  const fetchMyLeagueData = async () => {
      try {
          ...
          setLeague(data);
          if (data) setManualGw(data.currentGw);
  ...
  const handleGenerateFixtures = async () => { try { if (!league) return; if(!window.confirm('توليد جدول مباريات جديد؟ سيتم مسح الجدول القديم!')) return; const { data } = await API.post('/fixtures/generate', { leagueId: league._id }); setMessage(data.message); } catch (err) { setMessage('خطأ'); } };
  ...
  const handleShareWhatsapp = () => { const shareText = league ? `انضم إلينا في بطولة ${league.name} 🏆\nالكود: *${league.code}*` : ''; const url = `https://wa.me/?text=${encodeURIComponent(shareText)}`; window.open(url, '_blank'); };
  ...
  const renderRewardNotice = () => {
    ...
    return (
        <div style={{ background: 'linear-gradient(135deg, #FFD700 0%, #FFA000 100%)', color: '#38003c', padding: isMobile ? '15px' : '20px', borderRadius: '15px', marginBottom: '20px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: 'center', gap: '15px', boxShadow: '0 10px 25px rgba(255, 215, 0, 0.4)', textAlign: isMobile ? 'center' : 'right' }}>
            <div style={{ fontSize: isMobile ? '35px' : '45px' }}>🏆</div>
            <div style={{ flex: 1 }}><h3 style={{ margin: '0 0 5px 0', fontSize: isMobile ? '18px' : '22px', fontWeight: '900' }}>👑 مبروك! بطل الجولة {league.lastGwWinner.gameweek}</h3><p style={{ margin: 0, fontWeight: 'bold', fontSize: isMobile ? '14px' : '16px' }}>حقق فريقكم ({league.lastGwWinner.points}) نقطة. تمت إضافة "نقطة ذهبية" ✨</p></div>
        </div>
    );
  };
  ...
  const renderPenaltyNotice = () => {
    ...
    return (
        <div style={{ backgroundColor: config.bg, color: config.color, padding: '15px', borderRadius: '12px', marginBottom: '25px', border: `2px solid ${config.color}`, display: 'flex', alignItems: 'center', gap: '15px' }}>
            <div style={{ fontSize: '30px' }}>{config.icon}</div>
            <div style={{ flex: 1 }}><h3 style={{ margin: '0 0 5px 0', fontSize: '16px' }}>{config.title}</h3><p style={{ margin: 0, fontSize: '13px', fontWeight:'bold' }}>{config.text}</p></div>
        </div>
    );
  };
    ...
    <div style={{ padding: isMobile ? '20px' : '40px', fontFamily: 'Arial, sans-serif', direction: 'rtl', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <header style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', marginBottom: '30px', borderBottom: '1px solid #ddd', paddingBottom: '20px', gap: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            {league?.logoUrl && <img src={`${SERVER_URL}${league.logoUrl}`} alt="L" style={{ width: '55px', height: '55px', borderRadius: '50%', border: '2px solid #38003c', backgroundColor: 'white' }} />}
            <div>
                <h1 style={{ margin: 0, fontSize: isMobile ? '22px' : '28px' }}>لوحة التحكم 📱</h1>
                <p style={{ margin: '5px 0', color: '#666', fontSize: '13px' }}>أهلاً، {user.username} {league && <span style={{ color: '#1565c0', fontWeight: 'bold' }}> • GW {league.currentGw}</span>}</p>
            </div>
        </div>
        ...
        <div style={{ marginBottom: '30px' }}>
            ...
            <div onClick={() => navigate(`/team-history/${nextOpponent.opponent._id}`, { state: { team: nextOpponent.opponent, startGw: nextOpponent.gameweek } })} style={{ background: 'linear-gradient(135deg, #6a1b9a 0%, #4a148c 100%)', color: 'white', padding: '15px', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', boxShadow: '0 5px 15px rgba(0,0,0,0.2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ background: 'white', padding: '5px', borderRadius: '50%', width: '50px', height: '50px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>{nextOpponent.opponent.logoUrl && <img src={nextOpponent.opponent.logoUrl} style={{ width: '35px', height: '35px', objectFit: 'contain' }} />}</div>
                    <div><div style={{ fontSize: '11px', opacity: 0.8 }}>{nextOpponent.isHome ? 'بملعبك' : 'خارج ملعبك'} • GW{nextOpponent.gameweek}</div><div style={{ fontSize: '18px', fontWeight: 'bold' }}>{nextOpponent.opponent.name}</div></div>
                </div>
      ...
      {/* Admin Tools - تم إعادة الأزرار المحذوفة هنا */}
      {user.role === 'admin' && (
        <div style={{ backgroundColor: '#f3e5f5', padding: isMobile ? '15px' : '25px', borderRadius: '15px', marginBottom: '40px', border: '1px solid #ce93d8' }}>
          <h3 style={{ marginTop: 0, color: '#38003c', textAlign: 'center' }}>🛠 إدارة البطولة (المدير)</h3>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '20px' }}>
              
             <div style={{ backgroundColor: 'white', padding: '15px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <h4 style={{marginTop:0, fontSize: '15px', color: '#1565c0'}}>1. الإعدادات والتحكم بالجولة</h4>
              {league ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      <div style={{ background: '#f9f9f9', padding: '10px', borderRadius: '8px', textAlign: 'center' }}>
                          <span style={{fontSize: '11px', display: 'block'}}>كود الانضمام:</span>
                          <strong style={{fontSize: '18px', color: '#38003c'}}>{league.code}</strong>
                          <div style={{display:'flex', justifyContent:'center', gap:'5px', marginTop:'5px'}}>
                             <button onClick={handleCopyCode} style={{padding:'4px 8px', fontSize:'10px'}}>نسخ</button>
                             <button onClick={handleShareWhatsapp} style={{padding:'4px 8px', fontSize:'10px', background:'#25D366', color:'white', border:'none'}}>واتساب</button>
                          </div>
                      </div>
                      
                      <div style={{ padding: '10px', background: '#fff9c4', borderRadius: '8px', border: '1px solid #fbc02d' }}>
                          <label style={{fontSize:'12px', fontWeight:'bold', display:'block', marginBottom:'5px'}}>تعديل الجولة يدوياً:</label>
                          <div style={{display:'flex', gap:'5px'}}>
                              <input type="number" value={manualGw} onChange={(e) => setManualGw(e.target.value)} style={{width:'60px', padding:'5px'}} />
                              <button onClick={handleSetGameweek} style={{flex:1, background:'#fbc02d', border:'none', borderRadius:'4px', fontWeight:'bold'}}>تحديث GW</button>
                          </div>
                      </div>

                      <button onClick={handleGenerateFixtures} style={{ padding: '10px', background: '#e91e63', color: 'white', border: 'none', borderRadius: '5px', fontWeight: 'bold', fontSize: '12px' }}><FaMagic /> توليد جدول المباريات</button>
                      <button onClick={handleSyncPlayers} style={{ padding: '10px', background: '#009688', color: 'white', border: 'none', borderRadius: '5px', fontWeight: 'bold', fontSize: '12px' }}><FaListOl /> مزامنة أهداف اللاعبين</button>
                  </div>
              ) : (
                  <div style={{display:'flex', gap:'5px'}}>
            ...
            <div style={{ backgroundColor: 'white', padding: '15px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <h4 style={{marginTop:0, fontSize: '15px', color: '#2e7d32'}}>2. إدارة النتائج والبيانات</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <button onClick={handleSyncGameweeks} style={{ padding: '10px', background: '#38003c', color: '#00ff85', border: 'none', borderRadius: '5px', fontWeight: 'bold', fontSize: '12px' }}><FaSync /> مزامنة مواعيد المباريات</button>
                  ...
                  <div style={{ padding: '8px', border: '1px solid #c62828', borderRadius: '8px', background: '#ffebee' }}>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#c62828', marginBottom: '5px' }}>استيراد العقوبات (Excel):</div>
                    <div style={{ display: 'flex', gap: '5px' }}>
                        <input type="file" onChange={(e) => setPenaltyExcelFile(e.target.files[0])} style={{ fontSize: '10px', flex: 1 }} />
                        <button onClick={handleImportPenalties} style={{ padding: '4px 8px', background: '#c62828', color: 'white', border: 'none', borderRadius: '4px', fontSize: '10px' }}>رفع</button>
                    </div>
                  </div>

                  <button onClick={handleCalculateScores} style={{ padding: '10px', background: '#2196f3', color: 'white', border: 'none', borderRadius: '5px', fontWeight: 'bold' }}>1. حساب النقاط</button>
                  <button onClick={handleUpdateTable} style={{ padding: '10px', background: '#673ab7', color: 'white', border: 'none', borderRadius: '5px', fontWeight: 'bold' }}>2. تحديث الترتيب</button>
              </div>
            </div>
            ...
            <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '15px' }}>
                <div style={{ flex: 1, backgroundColor: '#fff3e0', padding: '12px', borderRadius: '10px', border: '1px solid #ffcc80' }}>
                    <h4 style={{ margin: '0 0 10px 0', fontSize: '14px' }}>🔔 طلبات الفرق الجدد</h4>
                ...
                <div style={{ flex: 1, backgroundColor: '#e3f2fd', padding: '12px', borderRadius: '10px', border: '1px solid #90caf9' }}>
                    <h4 style={{ margin: '0 0 10px 0', fontSize: '14px' }}>🔄 طلبات تغيير المناجير</h4>
                    {pendingSubs.length === 0 ? <p style={{fontSize:'12px', textAlign:'center'}}>لا توجد طلبات</p> : (
                        <table style={{ width: '100%', fontSize: '12px' }}><tbody>{pendingSubs.map(team => (<tr key={team._id}><td style={{padding:'5px'}}>{team.name}</td><td style={{textAlign:'left'}}><div style={{display:'flex', gap:'5px'}}><button onClick={() => handleApproveSub(team._id)} style={{background:'#2e7d32', color:'white', border:'none', padding:'4px 8px', borderRadius:'4px'}}><FaCheck/></button></div></td></tr>))}</tbody></table>
                    )}
                </div>
            </div>
          </div>
                    ...
                    <div style={{ background: '#f5f5f5', padding: '20px', borderRadius: '15px', textAlign: 'center' }}>
                        ...
                        {!selectionMode && (
                            <div style={{ display:'flex', gap:'10px', flexDirection:'column' }}>
                                <button onClick={() => { setSelectionMode('create'); fetchPLTeams(); }} style={{ padding:'12px', background:'#38003c', color:'white', border:'none', borderRadius:'10px', fontWeight:'bold' }}>أنا مناجير (إنشاء فريق)</button>
