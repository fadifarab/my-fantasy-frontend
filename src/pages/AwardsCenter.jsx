...
const AwardsCenter = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                // ✅ تعديل: ضبط الجولة الحالية مباشرة لتظهر أولاً
                setGw(currentGwId); 
    ...
    const DreamTeamPitch = ({ players }) => {
        ...
        return (
            <>
                {/* الحاوية الخارجية للملعب لتسهيل التجاوب */}
                <div className="pitch-wrapper" style={{ width: '100%', overflowX: 'auto', padding: '10px 0' }}>
                    <div style={{ 
                        background: 'linear-gradient(180deg, #1b5e20 0%, #2e7d32 50%, #1b5e20 100%)', 
                        padding: '20px 10px 40px 10px', 
                        borderRadius: '15px', border: '4px solid #ffd700', 
                        margin: '20px auto', minHeight: '700px', width: '100%', maxWidth: '600px', position: 'relative', overflow: 'hidden',
    ...
    return (
        <div style={{ padding: '10px', background: '#f5f7fa', minHeight: '100vh', fontFamily: 'Arial', direction: 'rtl' }}>
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px', gap: '15px' }}>
                ...
                <h1 style={{ margin: 0, color: '#38003c', fontSize: '20px' }}>🏆 مركز الجوائز</h1>
            </div>

            {/* Tabs - Mobile Friendly */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '5px', marginBottom: '20px', flexWrap:'nowrap', overflowX: 'auto', paddingBottom: '10px' }}>
                <button onClick={() => setActiveTab('gameweek')} style={{ flexShrink:0, padding: '10px 15px', borderRadius: '20px', border: 'none', background: activeTab === 'gameweek' ? '#38003c' : 'white', color: activeTab === 'gameweek' ? 'white' : '#333', fontWeight: 'bold', fontSize: '13px' }}>🌟 الجولة</button>
                <button onClick={() => setActiveTab('month')} style={{ flexShrink:0, padding: '10px 15px', borderRadius: '20px', border: 'none', background: activeTab === 'month' ? '#38003c' : 'white', color: activeTab === 'month' ? 'white' : '#333', fontWeight: 'bold', fontSize: '13px' }}>📅 الشهر</button>
                <button onClick={() => setActiveTab('season')} style={{ flexShrink:0, padding: '10px 15px', borderRadius: '20px', border: 'none', background: activeTab === 'season' ? '#38003c' : 'white', color: activeTab === 'season' ? 'white' : '#333', fontWeight: 'bold', fontSize: '13px' }}>👑 الموسم</button>
                <button onClick={() => setActiveTab('form')} style={{ flexShrink:0, padding: '10px 15px', borderRadius: '20px', border: 'none', background: activeTab === 'form' ? '#38003c' : 'white', color: activeTab === 'form' ? 'white' : '#333', fontWeight: 'bold', fontSize: '13px' }}>📊 الفورمة</button>
            </div>
                ...
                <>
                    {(activeTab === 'gameweek' || activeTab === 'season' || activeTab === 'month') && (
                        <div>
                             {/* تحكم الجولة */}
                             {activeTab === 'gameweek' && (
                                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '15px', fontSize: '18px', fontWeight: 'bold' }}>
                                    ...
                                    <span style={{background:'#e0e0e0', padding:'5px 20px', borderRadius:'10px'}}>GW {gw}</span>
                            ...
                            {activeTab === 'month' && monthsList.length > 0 && (
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '20px' }}>
                                    <button onClick={() => changeMonth(-1)} disabled={selectedMonthIndex === 0} style={{ border: '1px solid #ddd', background: 'white', borderRadius: '50%', width:'35px', height:'35px', cursor: 'pointer' }}><FaChevronRight /></button>
                                    <div style={{ textAlign: 'center' }}>
                                        <select value={selectedMonthIndex} onChange={(e) => setSelectedMonthIndex(Number(e.target.value))} style={{ padding: '8px', borderRadius: '10px', border: '2px solid #38003c', fontWeight: 'bold' }}>
                                            {monthsList.map((m, i) => ( <option key={i} value={i}>{m.name}</option> ))}
                                        </select>
                                    </div>
                                    <button onClick={() => changeMonth(1)} disabled={selectedMonthIndex === monthsList.length - 1} style={{ border: '1px solid #ddd', background: 'white', borderRadius: '50%', width:'35px', height:'35px', cursor: 'pointer' }}><FaChevronLeft /></button>
                                </div>
                            )}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '15px', marginBottom: '30px' }}>
                                <div style={{ background: 'white', padding: '20px', borderRadius: '15px', textAlign: 'center', borderTop: '5px solid #00c853', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                                    <FaTrophy size={30} color="#00c853" />
                                    <h3 style={{ margin: '10px 0 5px 0' }}>{activeTab === 'month' ? 'بطل الشهر' : activeTab === 'season' ? 'بطل الموسم' : 'بطل الجولة'}</h3>
                                    {awards?.bestTeam ? (
                                        <div>
                                            <div style={{ fontSize: '18px', fontWeight: 'bold' }}>{awards.bestTeam.name}</div>
                                ...
                                <div style={{ background: 'white', padding: '20px', borderRadius: '15px', textAlign: 'center', borderTop: '5px solid #2979ff', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                                    <FaStar size={30} color="#2979ff" />
                                    <h3 style={{ margin: '10px 0 5px 0' }}>{activeTab === 'month' ? 'نجم الشهر' : 'نجم الجولة'}</h3>
                                    {awards?.bestPlayer ? (
                                        <div>
                                            <div style={{ display:'flex', justifyContent:'center', margin:'5px 0' }}><KitImage teamName={awards.bestPlayer.teamName} size={50} /></div>
                                            <div style={{ fontSize: '18px', fontWeight: 'bold' }}>{awards.bestPlayer.name}</div>
                    ...
                    {activeTab === 'form' && (
                        <div style={{ background: 'white', borderRadius: '15px', padding: '10px', overflowX: 'auto', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
                            <h3 style={{ borderBottom: '2px solid #38003c', paddingBottom: '10px' }}>📊 سجل الفورمة</h3>
                                    ...
                                    {formGuide.map((team, idx) => (
                                        <tr key={team.teamId} style={{ borderBottom: '1px solid #eee' }}>
                                            <td style={{ padding: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <img src={getLogoUrl(team.logoUrl)} style={{ width: '25px', height: '25px', objectFit: 'contain' }} />
                                                <span style={{ fontWeight: 'bold', fontSize: '13px' }}>{team.teamName}</span>
                                            </td>
